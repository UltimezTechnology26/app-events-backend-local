...
const { getIntValues, getPresentDateTime, array_column } = require('../../utils/helpers/helper')
...
import { buildWatchlistSearchConditions, buildWatchlistListPipeline, extractWatchlistPaginatedResult } from './company_watchlist.queries'
...
export async function addToWatchlist({ actor, companyRowIdRaw }: AddToWatchlistParams) {
  ...
  const checkCompany = await companyM.findOne({ _id: company_row_id, approval_status: 1, active_status: 1 }, { _id: 1 })
  if (!checkCompany) {
    return { status: false, message: { alert_message: 'Sorry, Invalid company row id.' } }
  }

  const nextId = await getCollectionID('cln_company_watchlists')
  const upsertResult: any = await company_watchlistM.findOneAndUpdate(
    { company_row_id, user_row_id },
    { $setOnInsert: { _id: nextId, company_row_id, user_row_id, last_email_sent_on: null, date_n_time: getPresentDateTime() } },
...
export async function removeFromWatchlist({ actor, companyRowIdRaw }: RemoveFromWatchlistParams) {
  ...
  const deleted = await company_watchlistM.findOneAndDelete({ company_row_id, user_row_id })
  if (!deleted) {
...
export async function getWatchlistList({ actor, skipRaw, limitRaw, search, businessModelIdRaw, location }: GetWatchlistListParams) {
  ...
  const aggregateOutput = await company_watchlistM.aggregate(buildWatchlistListPipeline({ userRowId: user_row_id, searchArray, skip, limit }))
 ...
 */
export const watchlistIds = async (headers: any) => {
