...
import { buildFundingRoundsListMatch, buildFundingRoundsListPipeline } from './funding.rounds.queries'
...
export async function getFundingRoundsList({ search, skipRaw, limitRaw }: GetFundingRoundsListParams) {
  ...
  const aggregateOutput = await funding_roundsM.aggregate(buildFundingRoundsListPipeline({ matchQuery, skip, limit }))
...
export async function saveOrUpdateFundingRound(body: SaveOrUpdateFundingRoundParams) {
  ...
  if (category_row_id > 0) {
    await funding_roundsM.updateOne({ _id: category_row_id }, { $set: { category_name: String(body.category_name).trim() } })
