...
import {
  buildFundingInvestorTypesListMatch,
  buildFundingInvestorTypesListPipeline,
  buildFundingInvestorTypeUsageCheckFilter,
...
export async function getFundingInvestorTypesList({ search, investorTypeRaw, skipRaw, limitRaw }: GetFundingInvestorTypesListParams) {
  ...
  const aggregateOutput = await funding_investor_typesM.aggregate(buildFundingInvestorTypesListPipeline({ matchQuery, skip, limit }))
...
export async function saveOrUpdateFundingInvestorType(body: SaveOrUpdateFundingInvestorTypeParams) {
  ...
  if (category_row_id) {
    await funding_investor_typesM.updateOne({ _id: category_row_id }, { $set: update_object })
  ...
  await new funding_investor_typesM(update_object).save()
  await invalidateFundingInvestorTypesCaches()
...
export async function deleteFundingInvestorType(categoryRowIdRaw: string) {
  ...
  const usageFilter = buildFundingInvestorTypeUsageCheckFilter(category_row_id)
  const checkCategoryPresent = await fundingInvestmentM.findOne(usageFilter)
