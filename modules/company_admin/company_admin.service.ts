...
const { getUpdateTrackerFields, calculateCompanyProfileScore } = require('../../utils/helpers/app_helper')
const { updateNotification } = require('../../utils/helpers/notification_helper')
const { deleteKeysByPattern } = require('../../config/cache_helper')
import { buildCompanyEventsCreatedPipeline, buildCompanyEventsSponsoredPipeline, buildSubadminOverviewPipeline } from './company_admin.queries'
...
export async function getSubadminList() {
  const message = await sub_adminM.find({ login_status: 1 }, { _id: 1, full_name: 1 }).sort({ full_name: 1 })
...
async function computeYearsOverview() {
  ...
  const [present_year, one_year_back, two_year_back, three_year_back, admin_present_year, admin_one_year_back, admin_two_year_back, admin_three_year_back] = await Promise.all([
    companyM.countDocuments({ active_status: 1, created_date_n_time: { $gte: new Date(present_year_date.start_date) } }),
    companyM.countDocuments({ active_status: 1, created_date_n_time: { $gte: new Date(one_year_back_date.start_date), $lte: new Date(one_year_back_date.end_date) } }),
    companyM.countDocuments({ active_status: 1, created_date_n_time: { $gte: new Date(two_year_back_date.start_date), $lte: new Date(two_year_back_date.end_date) } }),
    companyM.countDocuments({ active_status: 1, created_date_n_time: { $gte: new Date(three_year_back_date.start_date), $lte: new Date(three_year_back_date.end_date) } }),
    companyM.countDocuments({ sub_admin_row_id: { $gte: 1 }, active_status: 1, created_date_n_time: { $gte: new Date(present_year_date.start_date) } }),
    companyM.countDocuments({ sub_admin_row_id: { $gte: 1 }, active_status: 1, created_date_n_time: { $gte: new Date(one_year_back_date.start_date), $lte: new Date(one_year_back_date.end_date) } }),
    companyM.countDocuments({ sub_admin_row_id: { $gte: 1 }, active_status: 1, created_date_n_time: { $gte: new Date(two_year_back_date.start_date), $lte: new Date(two_year_back_date.end_date) } }),
    companyM.countDocuments({ sub_admin_row_id: { $gte: 1 }, active_status: 1, created_date_n_time: { $gte: new Date(three_year_back_date.start_date), $lte: new Date(three_year_back_date.end_date) } }),
  ])
...
export async function enableCompany({ admin, companyRowIdRaw }: EnableCompanyParams) {
  ...
  const check_access = await checkCompanySubadminAccess({
    admin_row_id: Number.parseInt(admin.message.admin_row_id),
    admin_manager_type: admin.message.admin_manager_type,
    sub_admin_type: Number.parseInt(admin.message.sub_admin_type),
    company_row_id,
  })
  ...
  const checkPodcast = await companyPodcastsM.findOne({ company_row_id })
  if (checkPodcast) {
    await companyPodcastsM.deleteOne({ company_row_id })
  }
...
export async function disableCompany({ admin, companyRowIdRaw, disableReason }: DisableCompanyParams) {
  ...
  if (admin.message.admin_manager_type == 2) {
    admin_row_id = admin.message.admin_row_id
  }
  ...
  await companyM.updateOne(
    { _id: company_row_id },
    { $set: { disable_reason: disableReason, disabled_sub_admin_row_id: admin_row_id, disabled_date_n_time: getPresentDateTime(), active_status: 0, ...updateFields } },
  )

  const checkEvents = await eventM.findOne({ company_row_id })
  ...
  if (checkEvents) {
    await eventM.updateMany({ company_row_id }, { $set: { active_status: 0 } })
  }
...
export async function bulkImportCompanies({ admin, bulkData }: BulkImportCompaniesParams) {
  ...
  for (const run of bulkData) {
    ...
    const check_name_query = await companyM.findOne({ company_name: sanitize(run.company_name) }).collation({ locale: 'en', strength: 2 })
    if (check_name_query) {
      insert_status = false
    }
    ...
    if (run.company_email_id) {
      ...
      if (check_email_query) {
        ...
        insert_array['company_email_id'] = (run.company_email_id as string).toLowerCase()
      }
    }
    ...
    const inserted_query: any = await companyM(insert_array).save()
    const company_row_id = inserted_query._id

    await company_seo_detailsM({
      company_row_id,
...
export interface UpdateCompanyPageDetailsParams {
  ...
  admin: Actor
  /** Normalized checkAllLoginToken result — the shape saveOrUpdateBasicCompanyDetails expects. */
  actor: any
  companyRowIdRaw: string
  body: Record<string, any>
  preValidationErrors: Record<string, any>
}
...
export async function updateCompanyPageDetails({ admin, actor, companyRowIdRaw, body, preValidationErrors }: UpdateCompanyPageDetailsParams) {
  ...
  if (result.status && company_logo) {
    const checkCompany = await companyM.findOne({ _id: company_row_id }, { _id: 1, company_logo: 1 })
    if (checkCompany?.company_logo) {
      await deleteImageDigitalOcean(checkCompany.company_logo, 1)
    }
    await companyM.updateOne({ _id: company_row_id }, { $set: { company_logo } })
  }
