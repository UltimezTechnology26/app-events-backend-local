...
const { getUpdateTrackerFields, deleteCompanyDetails } = require('../../utils/helpers/app_helper')
...
export async function approveCompanyRequest({ admin, requestRowIdRaw }: ApproveCompanyRequestParams) {
  const queryRun = await companyM.findOne({ _id: requestRowIdRaw })
  if (!queryRun) {
  ...
  const checkApprovalQuery = await companyM.findOne({ _id: requestRowIdRaw, approval_status: 0 })
  ...
  await companyM.updateOne({ _id: requestRowIdRaw }, { $set: { approval_status: 1, ...updateFields, updated_date_n_time: new Date() } })
