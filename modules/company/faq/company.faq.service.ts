...
import { buildFaqSearchMatch, buildFaqListPipeline } from './company.faq.queries'
...
export async function saveOrUpdateFaqDetails({ actor, body, preValidationErrors }: SaveFaqDetailsParams) {
  ...
  if (!Number.isNaN(Number.parseFloat(body.company_row_id))) {
    ...
    if (body.faq_row_id) {
      if (!Number.isNaN(Number.parseInt(body.faq_row_id))) {
        const check_valid_faq_query = await company_faqM.findOne({ _id: Number.parseInt(body.faq_row_id), company_row_id })
  ...
  await new company_faqM(update_object).save()
  await invalidateFaqCaches()
...
export async function getFaqList({ actor, companyRowIdRaw, skipRaw, limitRaw, search }: GetFaqListParams) {
  ...
  const aggregateOutput = await company_faqM.aggregate(buildFaqListPipeline({ searchMatch, skip, limit }))
