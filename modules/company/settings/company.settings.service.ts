...
import { buildIndividualDetailsPipeline, buildCompanyFollowersPipeline, buildGetCompanySeoPipeline } from './company.settings.queries'
import {
...
const { checkCompanyRowID, calculateCompanyProfileScore, getUpdateTrackerFields, deleteCompanyFollowers } = require('../../../utils/helpers/app_helper')
 ...
 */
async function notifySubAdminsOfNewCompany(subadminData: any[], emailData: { full_name: string; company_name: string; updated_date_n_time: any }) {
...
export async function saveOrUpdateBasicCompanyDetails({ actor, body, preValidationErrors }: SaveBasicCompanyDetailsParams) {
  ...
  if (actor.status) {
    ...
    if (actor.message.user_type == 1) {
      if (!company_row_id) {
        const check_user_company_query = await companyM.findOne({ user_row_id })
    ...
    if (body.company_id) {
      const check_company_id_query = await companyM.findOne({ $and: [{ _id: { $ne: company_row_id } }, { company_id: sanitize(body.company_id) }] }).collation({ locale: 'en', strength: 2 })
    ...
    if (body.company_email_id) {
      const check_email_id_query = await companyM.findOne({ $and: [{ _id: { $ne: company_row_id } }, { company_email_id: sanitize(body.company_email_id) }] }).collation({ locale: 'en', strength: 2 })
    ...
    if (body.contact_number) {
      ...
      const check_mobile_number_query = await companyM.findOne({ $and: [{ _id: { $ne: company_row_id } }, { contact_number: sanitize(body.contact_number) }] }).collation({ locale: 'en', strength: 2 })
  ...
  const insertArray: Record<string, any> = {
    business_model_id,
  ...
  const seoArray: Record<string, any> = {
    meta_keywords: body.meta_keywords,
    meta_description: body.meta_description,
    meta_title: body.meta_title,
  }
  const socialArray: Record<string, any> = { youtube_channel: body.youtube_channel }
  ...
  if (!company_row_id) {
    ...
    const hasChanged = (newVal: any) => (newVal ?? '').trim() !== ''
    await seo_change_logsM.create({
      ...
      user_type: actor.message.user_type === 1 ? 'user' : 'admin',
      updated_by: actor.message.user_type === 1 ? user_row_id : sub_admin_row_id,
    })
  ...
  const check_company_seo = await company_seo_detailsM.findOne(
    { company_row_id },
  ...
  const hasChangedPair = (oldVal: any, newVal: any) => (newVal ?? '').trim() !== '' && (oldVal ?? '').trim() !== (newVal ?? '').trim()
  const changed =
  ...
  await invalidateBasicDetailsCaches()
  await company_seo_detailsM.updateOne({ company_row_id }, { $set: seoArray }, { upsert: true })
...
export async function saveOrUpdateBasicCompanyDetailsTeamPanel({ body, preValidationErrors }: SaveBasicCompanyDetailsTeamPanelParams) {
  ...
  const seoArray: Record<string, any> = { meta_keywords: body.meta_keywords, meta_description: body.meta_description, meta_title: body.meta_title }
  const socialArray: Record<string, any> = { youtube_channel: body.youtube_channel }
  ...
  await companyM.updateOne({ _id: company_row_id }, { $set: insertArray })
  await company_seo_detailsM.updateOne({ company_row_id }, { $set: seoArray }, { upsert: true })
  await company_social_linksM.updateOne({ company_row_id }, { $set: socialArray }, { upsert: true })
  await invalidateBasicDetailsCaches()
...
export async function saveOrUpdateSocialDetails({ actor, body, preValidationErrors }: SaveSocialDetailsParams) {
  ...
  const insert_object: Record<string, any> = {
  ...
  const check_query = await company_social_linksM.findOne({ company_row_id }, { _id: 1 })
  let social_links_key_deleted
  if (check_query) {
    await company_social_linksM.updateOne({ company_row_id }, { $set: insert_object })
...
async function getCreatorContactInfoForPopup(userRowId: number, subAdminRowId: number) {
  ...
  if (userRowId) {
    const [userInfo] = await professionalsM.aggregate([
      { $match: { _id: userRowId } },
      { $limit: 1 },
  ...
  if (subAdminRowId) {
    const [subAdminInfo] = await sub_adminM.aggregate([
...
export async function getCompanyIndividualDetailsData({ company_row_id, includeAdminExtras }: { company_row_id: number; includeAdminExtras?: boolean }) {
  try {
    const get_query_array = await companyM.aggregate(buildIndividualDetailsPipeline(company_row_id))
    ...
    const result: Record<string, any> = {
      ...
      business_model_id: await Promise.resolve(getIntIdFromArray(get_query.business_model_id)),
    ...
    if (includeAdminExtras) {
      const [teamMembersList, revenueDetails, fundingList, investingList, creatorContact] = await Promise.all([
        getAdminTeamMembersListForPopup(get_query._id),
        getRevenueDetailsForPopup(get_query._id),
        getFundingListForPopup(get_query._id),
        getInvestingListForPopup(get_query._id),
        getCreatorContactInfoForPopup(get_query.user_row_id, get_query.sub_admin_row_id),
      ])
...
export async function followCompany({ checkUserToken, followingCompanyRowIdRaw }: FollowCompanyParams) {
  ...
  const queryList = await followersM.findOne({ company_row_id: following_company_row_id, user_row_id })
  if (queryList) {
...
export async function unfollowCompany({ checkUserToken, followingCompanyRowIdRaw }: UnfollowCompanyParams) {
  ...
  const user_row_id = checkUserToken.message
  const queryRun = await followersM.findOne({ user_row_id, company_row_id: following_company_row_id })
  if (!queryRun) {
...
export async function removeFollower({ checkUserToken, followerUserRowIdRaw }: RemoveFollowerParams) {
  ...
  const queryRun = await followersM.findOne({ user_row_id: follower_user_row_id, company_row_id })
  if (!queryRun) {
    return { status: false, message: { alert_message: 'Already removed..' } }
  }
...
export async function getCompanyFollowersList({ companyRowIdRaw }: { companyRowIdRaw: string }) {
  ...
  const get_query = await followersM.aggregate(buildCompanyFollowersPipeline(company_row_id))
...
export async function getCompanyWalletAddressList({ checkUserToken }: { checkUserToken: { status: boolean; message: any } }) {
  ...
  const user_row_id = checkUserToken.message
  const getCompany = await companyM.findOne({ user_row_id }, { _id: 1 })
  if (!getCompany) {
...
export async function addCompanyWalletAddress({ checkUserToken, body, preValidationErrors }: AddCompanyWalletAddressParams) {
  ...
  if (user_row_id) {
    const checkWalletAddress = await company_wallet_addressM.findOne({ wallet_address, company_row_id })
...
export async function verifyCompanyEmailOtp({ checkUserToken, body, preValidationErrors }: VerifyCompanyEmailOtpParams) {
  ...
  const check_email_query = await verify_emailM.findOne({ user_row_id, email_verify_code: token_res.email_verify_code })
  ...
  const query = await companyM.findOne({ user_row_id, approval_status: 1, active_status: 1 })
  resArray['company_listed_status'] = query ? 1 : 0

  const imageQueryRun = await professionals_profile_imagesM.findOne({ user_row_id })
  resArray['profile_image'] = imageQueryRun ? imageQueryRun.profile_image : ''

  await verify_emailM.updateOne({ user_row_id }, { $set: { email_otp_number: '', email_verify_code: '', email_verify_status: true } })
...
export async function sendEmailOtp({ checkUserToken }: { checkUserToken: { status: boolean; message: any } }) {
  ...
  const user_row_id = checkUserToken.message
  const user_query = await professionalsM.findOne({ _id: user_row_id, email_verify_status: 1, login_status: true }, { _id: 1, full_name: 1, email_verify_status: 1 })
  if (!user_query) {
  ...
  const company_query = await companyM.findOne({ user_row_id }, { _id: 1, company_email_id: 1, email_verify_status: 1, company_name: 1 })
  if (!company_query) {
  ...
  await companyM.updateOne({ user_row_id }, { email_verify_status: false, email_verify_otp: verify_otp })
...
export async function updateCompanySeo({ actor, body, preValidationErrors }: UpdateCompanySeoParams) {
  ...
  if (seoChanged) {
    await seo_change_logsM.create({
      ...
      user_type: actor.message.user_type == 1 ? 'user' : 'admin',
      updated_by: actor.message.user_row_id || 0,
    })
  }
  ...
  const updateResult = await company_seo_detailsM.updateOne({ company_row_id: Number(module_id) }, { $set: updateData }, { upsert: true })
...
/** Ports setting.js's GET /get_company_seo/:company_id (lines 2354-2527) — confirmed the live, canonical implementation (both frontend-appcp-typescript and admin-coinpedia call this path exclusively). A separate, same-shaped implementation in controllers/app/analytics.js at a different, unmounted-by-any-consumer URL is left untouched, not deleted, per the user's direction. */
export async function getCompanySeo({ actor, companyId }: GetCompanySeoParams) {
  ...
  const companyData = await companyM.aggregate(buildGetCompanySeoPipeline(condition))
