...
import { buildClaimRequestsListPipeline, buildClaimRequestsMatchQuery, extractClaimRequestsPaginatedResult } from './company_claim_requests.queries'
...
async function addNewClaimRequest(userRowId: number, companyRowId: number, claimType: number) {
  const existing = await company_claim_requestsM.findOne({ user_row_id: userRowId, company_row_id: companyRowId, claim_status: 1 })
  if (existing) {
    return false
  }

  return new company_claim_requestsM({
...
async function sendClaimRequestEmail(toEmail: string, companyName: string, companyId: string, claimantEmail: string, claimantFullName: string, dateNTime: any) {
...
export async function saveClaimRequestDetails({ actor, companyRowIdRaw }: SaveClaimRequestDetailsParams) {
  ...
  if (company_query) {
    ...
    const claim_details: any = await addNewClaimRequest(user_row_id, company_query._id, 1)
    if (!claim_details) {
  ...
  const company_same_domain_query = await companyM.findOne({ website_link: { $regex: email_id_domain, $options: 'i' }, _id: company_row_id, approval_status: 1, active_status: 1 })
  ...
  const claim_details: any = await addNewClaimRequest(user_row_id, company_same_domain_query._id, 2)
  if (!claim_details) {
    return { status: false, message: { alert_message: 'Sorry, Your company claim request is already in queue, please wait admin will be process your request.' } }
  }
...
export async function claimCompany({ actor, companyRowIdRaw }: ClaimCompanyParams) {
  ...
  const user_row_id = actor.message
  const checkUserListedCompany = await companyM.findOne({ user_row_id })
  if (checkUserListedCompany) {
    return { status: false, message: { alert_message: 'You already have a company' } }
  }

  const checkCompanyAdCreated = await company_created_by_adminM.findOne({ company_row_id })
  ...
  await company_created_by_adminM.updateOne({ company_row_id }, { $set: { claim_verify_code: rndm_string } })
...
export async function verifyClaim({ actor, claimTokenRaw }: VerifyClaimParams) {
  ...
  const caller_user_row_id = Number.parseInt(actor.message)
  ...
  const checkUser = await professionalsM.findOne({ _id: token_user_row_id })
  if (!checkUser) {
  ...
  const saveData = await company_created_by_adminM.findOne({ company_row_id, claim_verify_code: claim_token.claim_verify_code })
  if (!saveData) {
    return { status: false, message: { alert_message: 'Sorry! Invalid Claim Token' } }
  }
  ...
  await company_created_by_adminM.updateOne({ company_row_id }, { $set: { claim_status: 2 } })

  const updateFields = getUpdateTrackerFields(actor)
  await companyM.updateOne({ _id: company_row_id }, { $set: { user_row_id: token_user_row_id, ...updateFields } })
...
export async function getClaimRequestsList({ claimStatusRaw, skipRaw, limitRaw, search }: GetClaimRequestsListParams) {
  ...
  const aggregateOutput = await company_claim_requestsM.aggregate(buildClaimRequestsListPipeline({ claimStatus, matchQuery, skip, limit }))
...
export async function approveClaimRequest({ admin, requestRowIdRaw }: ApproveClaimRequestParams) {
  ...
  const queryRun = await company_claim_requestsM.aggregate([
    { $match: { _id: request_row_id, claim_status: 1 } },
  ...
  const check_user_created_query = await companyM.findOne({ user_row_id })
  ...
  await companyM.updateOne({ _id: company_row_id }, { $set: { user_row_id, claim_status: 2, ...updateFields } })
  await company_created_by_adminM.updateOne({ company_row_id }, { $set: { claim_status: 2 } })
  await company_claim_requestsM.updateOne({ _id: request_row_id }, { $set: { claim_status: 2, claim_action_date_n_time: getPresentDateTime() } })
...
export async function rejectClaimRequest({ admin, requestRowIdRaw, rejectedReason }: RejectClaimRequestParams) {
  const queryRun = await company_claim_requestsM.aggregate([
    ...
    { $lookup: { from: 'cln_company_lists', localField: 'company_row_id', foreignField: '_id', as: 'company_info' } },
    { $unwind: { path: '$company_info', preserveNullAndEmptyArrays: true } },
    { $match: { _id: Number.parseInt(requestRowIdRaw), claim_status: 1 } },
    {
      $project: {
        _id: 1,
        user_row_id: 1,
  ...
  await company_claim_requestsM.updateOne(
    { _id: requestRowIdRaw },
    { $set: { claim_status: 3, claim_rejected_reason: rejectedReason, claim_action_date_n_time: getPresentDateTime() } },
  )
