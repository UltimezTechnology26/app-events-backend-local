...
router.get('/list', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const queryRun = await jobs_roleM.find()
        ...
        console.log('Job roles list.', err.message)
    ...
    try {
        ...
        if (checkToken.status) {
            if (Object.keys(errObj).length > 0) {
                ...
                await jobs_roleM({ category_name: req.body.category_name, active_status: true, date_n_time: getPresentDateTime() }).save()
        ...
        console.log('Save Job roles.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
    ...
    try {
        ...
        if (checkToken.status) {
            if (Object.keys(errObj).length > 0) {
                ...
                await jobs_roleM.updateOne({ _id: request_row_id }, { category_name: req.body.category_name })
                await deleteKeysByPattern('job_list_*')
        ...
        console.log('Update Job roles.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/enable/:request_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            ...
            const checkQuery = await jobs_roleM.findOne({ _id: request_row_id, active_status: false })
            if (checkQuery) {
                await jobs_roleM.updateOne({ _id: request_row_id }, { $set: { active_status: true } })
        ...
        console.log('Enable Job roles.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/disable/:request_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const request_row_id = Number.parseInt(req.params.request_row_id)
            const checkQuery = await jobs_roleM.findOne({ _id: request_row_id, active_status: true })
            if (checkQuery) {
                await jobs_roleM.updateOne({ _id: request_row_id }, { $set: { active_status: false } })
        ...
        console.log('Disable Job roles.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/delete/:request_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const request_row_id = Number.parseInt(req.params.request_row_id)
            const checkQuery = await jobs_roleM.findOne({ _id: request_row_id })
            if (checkQuery) {
                await jobs_roleM.deleteOne({ _id: request_row_id })
                await deleteKeysByPattern('job_list_*')
        ...
        console.log('Delete Job roles.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
