...
router.get('/list', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const queryRun = await job_genderM.find({ active_status: true }, { _id: 1, category_name: 1, active_status: 1, date_n_time: 1 })
        ...
        console.log('Job gender list.', err.message)
    ...
    try {
        ...
        if (checkToken.status) {
            if (Object.keys(errObj).length > 0) {
                ...
                const checkCategory = await job_genderM.findOne({ _id: sanitize(req.body.category_id) })
                if (checkCategory) {
                    await job_genderM.updateOne({ _id: req.body.category_id }, { $set: { category_name: req.body.category_name, date_n_time: getPresentDateTime() } })
                ...
                await job_genderM({ category_name: req.body.category_name, active_status: true, date_n_time: getPresentDateTime() }).save()
        ...
        console.log('save and update Job gender .', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/enable/:category_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            ...
            const checkQuery = await job_genderM.findOne({ _id: category_row_id, active_status: false })
            if (checkQuery) {
                await job_genderM.updateOne({ _id: category_row_id }, { $set: { active_status: true } })
        ...
        console.log('Enable Job gender.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/disable/:category_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const category_row_id = Number.parseInt(req.params.category_row_id)
            const checkQuery = await job_genderM.findOne({ _id: category_row_id, active_status: true })
            if (checkQuery) {
                await job_genderM.updateOne({ _id: category_row_id }, { $set: { active_status: false } })
        ...
        console.log('Disable Job gender.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
...
router.get('/delete/:category_row_id', async (req, res) => {
    try {
        ...
        if (checkToken.status) {
            const category_row_id = Number.parseInt(req.params.category_row_id)
            const checkQuery = await job_genderM.findOne({ _id: category_row_id })
            if (checkQuery) {
                await job_genderM.deleteOne({ _id: category_row_id })
        ...
        console.log('Delete Job gender list.', err.message)
        res.json({ status: false, message: 'An unexpected error occurred. Please try again later.' })
    }
})
