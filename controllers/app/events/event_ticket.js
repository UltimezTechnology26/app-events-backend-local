 ...
 * `info_work` lookup inside GET /notify_users_list/:event_row_id. Resolves
 ...
 * `work_position` from `$info_work.position_name` — unchanged shape. `{ $limit: 1 }`
 * kept in its original position: after position resolution, before company lookups.
 */
function buildNotifyUsersInfoWorkPipeline() {
    return [
                    ...
                    const queryRun = await notify_userM.aggregate([
                        ...
                        { $unwind: { path: "$user_info", preserveNullAndEmptyArrays: true } },
                        {
                            $lookup:
                            {
                                ...
                                pipeline: buildNotifyUsersInfoWorkPipeline(),
                                as: "info_work",
                            }
                        },
...
router.buildNotifyUsersInfoWorkPipeline = buildNotifyUsersInfoWorkPipeline
