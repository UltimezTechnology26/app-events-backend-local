...
const { setCache, getCache, deleteCache, getKeys, deleteKeysByPattern } = require('../../../config/cache_helper')
...
const { addManualPosition, deleteProfessionalDetails, basic_details_points, social_details_points, getUserProfileWithScore, sendJobEligibilityEmail, sendNewsCoverageEmail, calculateUserProfileScore, calculateCompanyProfileScore, getUpdateTrackerFields } = require('../../../utils/helpers/app_helper')
        ...
        if (checkToken.status) {
            ...
            if (!Number.isNaN(professional_details_id)) {
                const get_query = await professionals_work_experienceM.aggregate([
                    { $match: { _id: professional_details_id, user_row_id: user_row_id, user_account_type: 1 } },
                    {
                        $lookup:
                        {
                            ...
                            pipeline: [
                                {
                                    $match: {
                                        $expr: {
                                            $in: [
                                                "$_id",
                                                { $map: { input: "$$positions", as: "p", in: "$$p.position_row_id" } }
                                            ]
                                        }
                                    }
                                },
                            ...
                            pipeline: [
                                {
                                    $match: {
                                        $expr: {
                                            $in: [
                                                "$_id",
                                                { $map: { input: "$$positions", as: "p", in: "$$p.sub_position_row_id" } }
                                            ]
                                        }
                                    }
                                },
                                                    ...
                                                    $arrayElemAt: [
                                                        {
                                                            $map: {
                                                                input: {
                                                                    $filter: {
                                                                        input: "$resolved_manual_positions",
                                                                        cond: { $eq: ["$$this._id", "$$p.sub_position_row_id"] }
                                                                    }
                                                                },
                                                                in: "$$this.position_name"
                                                            }
                                                        },
                                                        0
                                                    ]
                                                    ...
                                                    $arrayElemAt: [
                                                        {
                                                            $map: {
                                                                input: {
                                                                    $filter: {
                                                                        input: "$resolved_static_positions",
                                                                        cond: { $eq: ["$$this._id", "$$p.position_row_id"] }
                                                                    }
                                                                },
                                                                in: "$$this.position_name"
                                                            }
                ...
                if (get_query[0]) {
                    ...
                    result['positions'] = get_query[0].positions?.length > 0
                        ...
                        : [
                            // fallback for old records not yet touched by the cron —
                            // reconstructs a single-entry array from the denormalized fields
                            {
...
router.post('/update_professional_details', [
    ...
    try {
        ...
        if (checkUserToken.status) {
            ...
            // Frontend sends all positions as one flat array.
            // positions[0] is denormalized into position_row_id/position_type/
            // sub_position_row_id for backward compat with existing filter queries.
            let positions = []
            ...
            if (!raw_positions) {
                ...
                if (!Array.isArray(raw_positions) || raw_positions.length === 0) {
                    ...
                    for (const ap of raw_positions) {
                        ...
                        // Duplicate check within the array
            ...
            if (!professional_details_id) {
                const check_time_range_query = await professionals_work_experienceM.findOne({
                    ...
                    start_date: { $lte: sanitize(req.body.start_date) },
                    end_date: { $gte: sanitize(req.body.end_date) }
                }, { _id: 1 })
            ...
            if (Object.keys(errObj).length > 0) {
                ...
                // ── Derive denormalized primary position from positions[0] ───
                // Keeps all existing filter queries on position_row_id working
                // without any changes elsewhere in the codebase.
                ...
                insert_array['start_date'] = req.body.start_date
                insert_array['location_type'] = req.body.location_type
                insert_array['end_date'] = till_date_status === 1 ? req.body.end_date : ""
                insert_array['public_view'] = req.body.public_view ? req.body.public_view : false

                if (!professional_details_id) {
                    if (req.body.public_view == true) {
                        await professionals_work_experienceM.updateMany(
                            { user_row_id: user_row_id, user_account_type: 1 },
                    ...
                    await professionalsM.updateOne(
                        { _id: user_row_id },
                        { $set: { updated_date_n_time: getPresentDateTime() } }
                    )
                    ...
                    await calculateUserProfileScore(user_row_id, ['professional_detail'])
                    res.json({ status: true, message: "Your professional details have been successfully submitted. Thank you for sharing your valuable experience with us." })
                    ...
                    res.json({ status: true, check_work_query, till_date_status, message: "Your efforts to keep your information accurate are greatly appreciated. We have successfully updated your professional details." })
                }
            }
        } else {
            res.json({ status: false, message: { alert_message: checkUserToken.message } })
...
router.get('/professional_detail_list/:skip/:limit', async (req, res) => {
    const checkToken = checkUserLoginToken(req.headers)
    if (checkToken.status) {
        try {
