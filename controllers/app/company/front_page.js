...
router.get('/company_funding_details/:company_row_id', async (req, res) => {
    try {
        const company_row_id = Number.parseInt(req.params.company_row_id)
        const get_query = await companyM.findOne({ _id: company_row_id, approval_status: 1, active_status: 1 }, { _id: 1 })
        if (get_query) {

            let investment_search_query = [{}]
            let raised_search_query = [{
                verified_status: 1,
                funds_raised_registered_type: 1,
                funds_raised_company_row_id: company_row_id
            }]
            let sort_order = { announcement_date: -1 }
            if (req.query.investor_type == 1) {
                ...
                if (!Number.isNaN(Number.parseInt(req.query.sort_order))) {
                    if (req.query.sort_order == 1) {
                        sort_order = { amount: -1 }
                    }
                    else if (req.query.sort_order == 2) {
                        sort_order = { amount: 1 }
                    }
                }

                if (!Number.isNaN(Number.parseInt(req.query.raised_investor_type))) {
                    raised_search_query.push({ investor_type: Number.parseInt(req.query.raised_investor_type) })
                }

                if (req.query.category_row_id) {
                    raised_search_query.push({ category_row_id: Number.parseInt(req.query.category_row_id) });
                }
                if (req.query.search) {
                    raised_search_query.$or = [
                        { investor_name: { $regex: req.query.search, $options: "i" } },
                        { category_name: { $regex: req.query.search, $options: "i" } }
                    ];
                }
            }
            else if (req.query.investor_type == 2) {
                if (req.query.start_date) {
                    const start_date = createDateTime(req.query.start_date)
                    investment_search_query.push({ announcement_date: { $gte: new Date(start_date) } })
                }

                if (req.query.end_date) {
                    const end_date = createEndDateOnly(req.query.end_date)
                    investment_search_query.push({ announcement_date: { $lte: new Date(end_date) } })
                }
                if (!Number.isNaN(Number.parseInt(req.query.sort_order))) {
                    if (req.query.sort_order == 1) {
                        sort_order = { amount: -1 }
                ...
                if (req.query.category_row_id) {
                    investment_search_query.push({ category_row_id: Number.parseInt(req.query.category_row_id) });
                }


                if (req.query.investor_category_row_id) {
                    investment_search_query.push({ investor_category_row_id: Number.parseInt(req.query.investor_category_row_id) });
                }
            }
            // Build cache key dynamically
            const key = `company_investment_funding_${company_row_id}` +
                `_investorType_${req.query.investor_type || 'all'}` +
                `_start_${req.query.start_date || 'all'}` +
                `_end_${req.query.end_date || 'all'}` +
                `_sort_${req.query.sort_order || 'default'}` +
                `_raisedType_${req.query.raised_investor_type || 'all'}` +
                `_category_${req.query.category_row_id || 'all'}` +
                `_invCategory_${req.query.investor_category_row_id || 'all'}` +
                `_search_${req.query.search ? req.query.search.trim().toLowerCase() : 'none'}`;
            ...
            // INVESTMENT SIDE — UNCHANGED. Each row here represents one investment
            // this company made into another company's raised round. A company only
            // ever has one row per round it invested in (it doesn't get split across
            // multiple rows the way a raised round's investors do), so there is no
            // round-based duplication issue on this side.
            const company_invested_funds_query = fundingInvestmentM.aggregate([
                {
                    $match: {
                        verified_status: 1,
                        investor_type: 2,
            ...
            // RAISED SIDE — fixed to dedupe by round_id before summing, so a
            // multi-investor round's shared amount is counted once, not once per
            // investor row.
            const company_raised_funds_query = fundingInvestmentM.aggregate([
                {
                    $match: {
                        verified_status: 1,
                        funds_raised_registered_type: 1,
                ...
                // Collapse to one row per round_id before summing, so the round's
                // shared amount is counted exactly once regardless of investor count.
                {
                    $group: {
                        _id: "$round_id",
                        amount: { $first: "$amount" }
            ...
            // INVESTMENT LIST — adds is_syndicate detection via a self-lookup on
            // round_id, same mechanism used on investor_list and investor_overview.
            // Lets the frontend hide Edit/Delete and show the syndicate disclosure for
            // rounds where this company co-invested alongside other investors.
            const funds_investment_list_query = fundingInvestmentM.aggregate([
                {
                    $match: {
                        verified_status: 1,
                        investor_type: 2,
                ...
                ...(req.query.search
                    ? [{
                        $match: {
                            $or: [
                                { "company_data.company_name": { $regex: req.query.search, $options: 'i' } },
                                { "company_data.company_id": { $regex: req.query.search, $options: 'i' } },
                                { "category_info.category_name": { $regex: req.query.search, $options: 'i' } }
                            ]
                        }
                    }]
                    : []),
            ...
            // RAISED LIST — now grouped by round_id, with an investors[] array per
            // round, matching the shape used by the other funding list route. All the
            // existing $lookups are unchanged; only the final stages differ (group by
            // round instead of projecting one flat row per investor).
            const funds_raised_list_query = fundingInvestmentM.aggregate([
                ...
                ...(req.query.search
                    ? [{
                        $match: {
                            $or: [
                                { "investor_data.full_name": { $regex: req.query.search, $options: 'i' } },
                                { "investor_data.company_name": { $regex: req.query.search, $options: 'i' } },
                                { "category_info.category_name": { $regex: req.query.search, $options: 'i' } }
                            ]
                        }
                    }]
                ...
                },
                {
                    $project: {
                        ...
                        amount: 1,
                        category_name: "$category_info.category_name",
                        investor_position_name: {
                            $cond: {
                                if: { $gt: [{ $size: { $ifNull: ["$outer_info_work.positions", []] } }, 0] },
                                then: {
                                    $reduce: {
                                        input: "$outer_info_work.positions",
                                        initialValue: "",
                                        in: {
                                            $cond: {
                                                if: { $eq: ["$$value", ""] },
                                                then: "$$this.position_name",
                                                else: { $concat: ["$$value", ", ", "$$this.position_name"] }
                                            }
                                        }
                                    }
                                },
                                else: { $ifNull: ["$outer_info_work.position_name", ""] }
                            }
                        },
                        ...
                        investor_pro_batch: { $cond: { if: "$investor_data.pro_batch:", then: "$investor_data.pro_batch:", else: "" } },
                        investor_email_id: { $cond: { if: "$investor_data.email_id", then: "$investor_data.email_id", else: "$investor_data.company_email_id" } },
                        investor_category_name: "$investor_category_info.category_name",
                        investor_category_row_id: 1
                    }
                },
                // Group every investor row into its round. Round-level fields
                // (announcement_date, amount, category_row_id, category_name) are
                // identical across the round's rows, so $first is safe. Per-investor
                // fields go into the investors array — same shape as the other
                // funding list route.
                {
                    $group: {
                        _id: "$round_id",
                        ...
                        investors: {
                            $push: {
                                ...
                                investor_pro_batch: "$investor_pro_batch",
                                ...
                                investor_positions: "$outer_info_work.positions",
                            }
                        }
                    }
                },
            ...
            const [total_funds_invested, total_funds_raised, funds_investment_list, funds_raised_list] = await Promise.all([company_invested_funds_query, company_raised_funds_query, funds_investment_list_query, funds_raised_list_query])

            let result = {}
            result['total_funds_invested'] = total_funds_invested[0] ? total_funds_invested[0].total : 0
            result['total_funds_raised'] = total_funds_raised[0] ? total_funds_raised[0].total : 0
            result['funds_investment_list'] = funds_investment_list
            result['funds_raised_list'] = funds_raised_list
