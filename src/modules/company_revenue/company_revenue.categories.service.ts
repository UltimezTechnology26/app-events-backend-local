...
import {
  ...
  findRevenueStreamCategoryById,
  findDuplicateRevenueStreamCategoryName,
  updateRevenueStreamCategoryName,
  insertRevenueStreamCategory,
  deleteRevenueStreamCategoryById,
  findRevenueGrowthUsingCategory,
} from './company_revenue.categories.queries'
import { fetchRevenueStreamCategoriesList } from './company_revenue.categories.list.queries'
...
export async function getRevenueStreamCategoriesList({ search, skipRaw, limitRaw }: GetRevenueStreamCategoriesListParams) {
  ...
  const aggregateOutput = await fetchRevenueStreamCategoriesList(matchStage, skip, limit)
...
export async function saveNEditRevenueStreamCategory({ body, preValidationErrors }: SaveNEditRevenueStreamCategoryParams) {
  ...
  if (body.category_row_id) {
    ...
    const checkFunding = await findRevenueStreamCategoryById(category_row_id)
    if (!checkFunding) {
  ...
  if (body.category_name) {
    ...
    const duplicateQuery = await findDuplicateRevenueStreamCategoryName(category_row_id, category_name)
  ...
  if (category_row_id) {
    await updateRevenueStreamCategoryName(category_row_id, category_name)
  ...
  await insertRevenueStreamCategory(category_name, getPresentDateTime())
  await invalidateRevenueStreamCategoriesCaches()
...
export async function deleteRevenueStreamCategory(categoryRowIdRaw: string) {
  ...
  const checkQuery = await findRevenueStreamCategoryById(category_row_id)
  if (!checkQuery) {
    return { status: false, message: { alert_message: 'Sorry, Invalid revenue stream round id ' } }
  }

  const checkCategoryPresent = await findRevenueGrowthUsingCategory(category_row_id)
  if (checkCategoryPresent) {
    return { status: false, message: { alert_message: 'Sorry, revenue stream in use, cannot delete.' } }
  }

  await deleteRevenueStreamCategoryById(category_row_id)
  await invalidateRevenueStreamCategoriesCaches()
