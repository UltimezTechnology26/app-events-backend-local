...
export function buildCompaniesListMatchQuery({
  ...
  const and: Record<string, unknown>[] = [statusApply]
...
export function buildCompaniesListPipeline({
  matchQuery,
  categoryStatus,
  skip,
  limit,
}: {
  matchQuery: ReturnType<typeof buildCompaniesListMatchQuery>
...
export function extractCompaniesListResult(aggregateOutput: Parameters<typeof extractPaginatedResult>[0]) {
...
export function buildDeletedListPipeline({
  matchQuery,
  skip,
  limit,
}: {
  matchQuery: ReturnType<typeof buildDeletedListMatchQuery>
  skip: number
  limit: number
}) {
  return [
...
export function extractDeletedListResult(aggregateOutput: Parameters<typeof extractPaginatedResult>[0]) {
  return extractPaginatedResult(aggregateOutput)
}
...
/** Used by approveCompanyRequest/rejectCompanyRequest/deleteCompanyRequest to load the target company row. */
export async function findCompanyById(companyRowId: number | string) {
...
/** Used by approveCompanyRequest/rejectCompanyRequest to persist the new approval status/tracker fields. */
export async function updateCompanyApprovalFields(companyRowId: number | string, updateFields: Record<string, unknown>) {
