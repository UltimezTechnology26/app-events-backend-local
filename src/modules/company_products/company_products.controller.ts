...
import { getCompanyProductsList, getCompanyProductsListPublic, getExchangeCountryDetails, getOwnCompaniesList, getProductsForCompare, findTokenOwnedByUser } from './company_products.queries'
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #14.
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #15.
companyProductsRouter.get('/compare_company_products', async (req: Request, res: Response) => {
  try {
    let company_row_ids_raw: unknown = req.query.company_row_ids

    if (!company_row_ids_raw) {
      return res.json({ status: false, message: 'company_row_ids is required' })
    }

    if (typeof company_row_ids_raw === 'string') {
      try {
        company_row_ids_raw = JSON.parse(company_row_ids_raw)
      } catch {
        return res.json({ status: false, message: 'company_row_ids must be array' })
      }
    }

    if (!Array.isArray(company_row_ids_raw)) {
      return res.json({ status: false, message: 'company_row_ids must be array' })
    }

    const company_row_ids: number[] = (company_row_ids_raw as (string | number)[])
      .map((id: string | number) => Number.parseInt(id as string))
...
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #16.
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #17.
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #18.
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #19.
 ...
 */
// NOT migrated to asyncRoute — see docs/error-handling-exceptions.md #20.
companyProductsRouter.get('/company_products/own_companies_list/:product_type/:product_row_id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (product_row_id && product_type === 1 && user_row_id) {
      const check_company = await findTokenOwnedByUser({ product_row_id, user_row_id })
