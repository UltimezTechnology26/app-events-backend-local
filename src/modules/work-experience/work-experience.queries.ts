...
/**
 * Phase B (work-experience reconciliation, per plan doc). Ports
 * controllers/admin_panel/app/user/work_experiences.js's GET
 * /manual_professional_detail_list/:user_row_id/:skip/:limit (~8-178) — a company-grouped
 * employment-history view scoped to manual-retrieval professionals (`user_account_type: 2`),
 * genuinely distinct in shape from this module's existing `getAdminProfessionalDetailList`
 * (flat list, self-registered professionals). Kept as its own function rather than folded into
 * that one.
 *
 * CONFIRMED PERF FIX: legacy's first `$match` only filters `user_account_type: 2` — scoping to
 * this specific user (`user_row_id`) doesn't happen until a LATER `$match`, three stages after two
 * `$lookup`s have already run against every manual-retrieval work-experience row across every
 * professional. Moving the `user_row_id` filter into the first `$match` (alongside
 * `user_account_type`) makes both `$lookup`s run only against this one user's rows instead of the
 * whole collection — same result, far less work per call.
 */
export function getManualProfessionalDetailListPipeline(userRowId: number): object[] {
  return [
    { $match: { user_account_type: 2, user_row_id: userRowId } },
    {
      $lookup: {
        from: 'cln_static_professionals_work_positions',
        localField: 'position_row_id',
