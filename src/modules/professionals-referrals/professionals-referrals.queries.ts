// modules/professionals-referrals/professionals-referrals.queries.ts
// Ports the aggregation pipeline from controllers/app/users/referrals.js's GET /list/:skip/:limit
// verbatim — legacy's own file already carries two confirmed bug fixes from a prior engagement
// (date-range filter applied to both list and count queries; created_date_n_time falling back to
// updated_date_n_time / the legacy date_n_time field for old records) — no further changes needed.
export function buildReferralsListPipeline(matchQuery: Record<string, any>) {
  return [
    { $match: matchQuery },
    { $sort: { _id: -1 } },
    { $lookup: { from: 'cln_static_countries', localField: 'country_id', foreignField: '_id', as: 'cln_co' } },
    ...
    { $lookup: { from: 'cln_auth_verify_emails', localField: '_id', foreignField: 'user_row_id', as: 'verify_email' } },
        ...
        pipeline: [
          ...
          { $unwind: { path: '$info_position', preserveNullAndEmptyArrays: true } },
          {
            $lookup: {
              ...
              pipeline: [
                { $match: { $expr: { $in: ['$_id', { $map: { input: '$$positions', as: 'p', in: '$$p.position_row_id' } }] } } },
          ...
          },
          {
            $lookup: {
              ...
              pipeline: [
                { $match: { $expr: { $in: ['$_id', { $map: { input: '$$positions', as: 'p', in: '$$p.sub_position_row_id' } }] } } },
                { $project: { _id: 1, position_name: 1 } },
              ],
            },
          },
          ...
          { $project: { position_name: 1, company_name: 1, positions: 1 } },
        ],
        as: 'info_work',
      },
    },
