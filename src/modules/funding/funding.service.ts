...
export async function getInvestorList(params: { investor_type: number; investor_row_id: number; skip: number; limit: number; query: Record<string, any> }) {
  ...
  // PERF FIX: `search`/`company_id` matching needs company_name/category_name, which only
  // exist after the lookups below run — but every other filter (investor identity, dates,
  // investment_type, category_row_id, investor_category_row_id) targets raw fields already
  // present on the base document. Splitting them lets the raw-field match run FIRST, before
  // $sort and the 3 lookup stages, so those stages operate on this investor's own rows
  // instead of the entire collection (previously the case even when no search term was given).
  const earlyMatch: any = { investor_type: params.investor_type, investor_registered_type: 1, investor_row_id: params.investor_row_id }
  if (params.query.start_date) earlyMatch.announcement_date = { ...earlyMatch.announcement_date, $gte: new Date(createDateTime(params.query.start_date)) }
  if (params.query.end_date) earlyMatch.announcement_date = { ...earlyMatch.announcement_date, $lte: new Date(createEndDateOnly(params.query.end_date)) }
  if (!Number.isNaN(Number.parseInt(params.query.investment_type))) earlyMatch.investor_type = Number.parseInt(params.query.investment_type)
  if (params.query.category_row_id) earlyMatch.category_row_id = Number.parseInt(params.query.category_row_id)
  if (params.query.investor_category_row_id) earlyMatch.investor_category_row_id = Number.parseInt(params.query.investor_category_row_id)

  const lateMatch: any[] = [{ company_data: { $nin: ['', null] } }]
  if (params.query.search) {
    const escaped = escapeRegex(String(params.query.search))
    lateMatch.push({ $or: [{ company_name: { $regex: escaped, $options: 'i' } }, { company_id: { $regex: escaped, $options: 'i' } }, { category_name: { $regex: escaped, $options: 'i' } }] })
  }
  ...
  const [list, [countResult]] = await Promise.all([
    ...
    fundingInvestmentM.aggregate([...commonStages, { $count: 'count' }])
  ])
...
export async function getManualFundsRaisedList(params: { funds_raised_company_row_id: number; skip: number; limit: number }): Promise<{ list: any[]; count: number } | null> {
  ...
  const list = await fundingInvestmentM.aggregate([
    ...
    // dropUnresolved: false — see resolveInvestorStages' own doc comment (confirmed bug fix,
    // manual companies' investors are more likely to reference a since-deleted record than
    // registered companies' are; the funding record itself is still real and should show).
    ...resolveInvestorStages({ rich: true, dropUnresolved: false }),
  ...
  const countResult = await fundingInvestmentM.aggregate([
    earlyMatchStage,
    // Must match the data query's dropUnresolved setting exactly, or list.length and count
    // drift apart — the same class of bug this codebase has already fixed elsewhere via $facet.
    ...resolveInvestorStages({ rich: false, dropUnresolved: false }),
    { $count: 'count' }
  ])
...
export async function getManualInvestorList(params: { investor_type: number; investor_row_id: number; skip: number; limit: number; query: Record<string, any> }) {
  ...
  const commonStages: any[] = [
    ...
    { $lookup: { from: 'cln_static_funding_investor_types', localField: 'investor_category_row_id', foreignField: '_id', as: 'investor_category_info', pipeline: [{ $project: { category_name: 1 } }] } },
    { $unwind: { path: '$investor_category_info', preserveNullAndEmptyArrays: true } },
    // dropUnresolved: false — see resolveFundsRaisedCompanyStages' own doc comment (confirmed
