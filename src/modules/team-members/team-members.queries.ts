  ...
  const get_query_promise = professionals_work_experienceM.aggregate([
    ...
    },
    {
      $set: {
        ...
        full_name: '$user_data.full_name',
        pro_batch: '$user_data.pro_batch',
        // CONFIRMED BUG FIX: `query`'s $or searches `position_name` (among other fields),
        // but this list pipeline never set it before the $match below - the count pipeline
        // just above already does (see its own `$set` before its `$match`), so a
        // position-name search returned a nonzero count with an empty list. Matches the
        // same $cond the final $project below re-derives position_name with.
        position_name: { $cond: { if: { $eq: ['$position_type', 2] }, then: '$manual_position_info.position_name', else: '$info_position.position_name' } }
      }
    },
    { $match: { $and: query } },
