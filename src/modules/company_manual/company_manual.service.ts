...
import {
  ...
  findCompanyByName,
  findManualCompanyByName,
  findCompanyByEmail,
  findManualCompanyByEmail,
  findCompanyByWebsiteLink,
  findManualCompanyByWebsiteLink,
  createManualCompany,
  findManualCompanyById,
  updateManualCompanyLogo,
  findPendingManualCompanyById,
  rejectManualCompanyRecord,
  findRejectedManualCompanyById,
  revokeManualCompanyRecord,
  findManualCompanyForDeletion,
  deleteManualCompanyRecord,
  findFundsInvestedForManualCompany,
  findFundsRaisedForManualCompany,
  findUserFundsRaisedForManualCompany,
  findSponsorPartnerForManualCompany,
  findWorkExperienceForManualCompany,
  aggregatePendingManualCompanies,
  aggregateRejectedManualCompanies,
  aggregateApprovedManualCompanies,
  aggregateManualCompanyIndividualDetail,
  aggregateManualCompanyEmployeeList,
  aggregateManualCompanySponsorList,
  aggregateManualCompanyPartnerList,
...
import { AdminActor, AddManualCompanyDetailsBody, EditManualCompanyLogoBody, RejectManualCompanyBody } from './company_manual.types'

// ─── App-side ───────────────────────────────────────────────────────────────

export interface AddManualCompanyDetailsParams {
  body: AddManualCompanyDetailsBody
...
export async function addManualCompanyDetails({ body, preValidationErrors }: AddManualCompanyDetailsParams) {
  ...
  if (body.company_name) {
    ...
    const check_company_query = await findCompanyByName(company_name)
    if (check_company_query) {
      errObj['company_name'] = 'Sorry, This Company Name is already exist.'
    } else {
      const check_manual_company_query = await findManualCompanyByName(company_name)
  ...
  if (body.company_email_id) {
    ...
    const check_company_query = await findCompanyByEmail(company_email_id)
    if (check_company_query) {
      errObj['company_email_id'] = 'Sorry, This Company Email ID is already exist.'
    } else {
      const check_manual_company_query = await findManualCompanyByEmail(company_email_id)
      if (check_manual_company_query) {
        errObj['company_email_id'] = 'Sorry, This Company Email ID is already exist.'
      }
    }
  ...
  if (body.website_link) {
    ...
    const check_company_query = await findCompanyByWebsiteLink(website_link)
    if (check_company_query) {
      errObj['website_link'] = 'Sorry, This Website Link is already exist.'
    } else {
      const check_link_query = await findManualCompanyByWebsiteLink(website_link)
      if (check_link_query) {
        errObj['website_link'] = 'Sorry, This Website Link is already exist.'
      }
    }
  ...
  const created_from_type = Number.parseInt(String(body.created_from_type))
  const insert_query = await createManualCompany({
    company_name,
    company_email_id,
    company_logo,
    created_from_type: Number.isFinite(created_from_type) ? created_from_type : 1,
    website_link,
    used_counts: 0,
...
export interface EditManualCompanyLogoParams {
  body: EditManualCompanyLogoBody
  preValidationErrors: Record<string, string>
}
...
export async function editManualCompanyLogo({ body, preValidationErrors }: EditManualCompanyLogoParams) {
  ...
  const check_manual_query = await findManualCompanyById(company_row_id)
  ...
  await updateManualCompanyLogo(company_row_id, company_logo)
  const get_manual_query = await findManualCompanyById(company_row_id)
  await invalidateManualCompanyListCache()
...
export async function getPendingManualCompanies({ actor, skipRaw, limitRaw, search, createdFromType }: GetManualCompanyListParams) {
  ...
  const aggregateOutput = await aggregatePendingManualCompanies({ matchQuery, skip, limit })
...
export async function getRejectedManualCompanies({ actor, skipRaw, limitRaw, search, createdFromType, rejectType }: GetManualCompanyListParams) {
  ...
  const aggregateOutput = await aggregateRejectedManualCompanies({ matchQuery, skip, limit })
  const { data, count } = extractPaginatedResult(aggregateOutput)
...
export async function getApprovedManualCompanies({ actor, skipRaw, limitRaw, search, createdFromType }: GetManualCompanyListParams) {
  ...
  const aggregateOutput = await aggregateApprovedManualCompanies({ matchQuery, skip, limit })
  const { data, count } = extractPaginatedResult(aggregateOutput)
...
export async function getManualCompanyIndividualDetail({ actor, companyRowIdRaw }: GetManualCompanyIndividualDetailParams) {
  ...
  const get_query = await aggregateManualCompanyIndividualDetail(company_row_id)
...
export async function rejectManualCompany({ actor, companyRowIdRaw, body, preValidationErrors }: RejectManualCompanyParams) {
  ...
  let admin_row_id: number | undefined = 0
  if (actor.message.admin_manager_type === 2) {
    admin_row_id = actor.message.admin_row_id
  }
  if (Number.parseInt(String(actor.message.sub_admin_type)) === 2) {
  ...
  const check_query = await findPendingManualCompanyById(company_row_id)
  if (!check_query) {
    return { status: false, message: { alert_message: 'Invalid Manual Company Row ID.' } }
  }

  await rejectManualCompanyRecord(company_row_id, {
...
export async function revokeManualCompany({ actor, companyRowIdRaw }: RevokeManualCompanyParams) {
  ...
  const check_query = await findRejectedManualCompanyById(company_row_id)
  if (!check_query) {
    return { status: false, message: { alert_message: 'Invalid Manual Company Row ID' } }
  }

  await revokeManualCompanyRecord(company_row_id)
  await invalidateManualCompanyListCache()
...
export async function deleteManualCompany({ actor, companyRowIdRaw }: DeleteManualCompanyParams) {
  ...
  const check_query = await findManualCompanyForDeletion(company_row_id)
  if (!check_query) {
    return { status: false, message: { alert_message: 'Invalid Manual Company Row ID' } }
  }

  const funds_invested_query = await findFundsInvestedForManualCompany(company_row_id)
  ...
  const funds_raised_query = await findFundsRaisedForManualCompany(company_row_id)
  ...
  const user_funds_raised_query = await findUserFundsRaisedForManualCompany(company_row_id)
  ...
  const sp_query = await findSponsorPartnerForManualCompany(company_row_id)
  if (sp_query) {
    await deleteSponsorsPartners({ type: 2, account_type: 2, registered_type: 2, user_company_row_id: company_row_id })
  }

  const work_experience_query = await findWorkExperienceForManualCompany(company_row_id)
  ...
  await deleteManualCompanyRecord(company_row_id)
  await invalidateManualCompanyListCache()
